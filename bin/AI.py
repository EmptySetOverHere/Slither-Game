...
def deriv_cosprox(x, y):
    ...
    # dL/dyp
    fx = cos_prox(x, y)
    z = y/(np.linalg.norm(x) * np.linalg.norm(y)) - fx * (x/np.sum(np.square(x)))
    return z
...
class slitherAI:
    ...
    def __init__(self, hidden_layers = None, output_layer = None, learning_rate = 0.1):
        
        self.maximum_data_size = 50
        self.no_of_hidden_layers = 2
        self.no_of_neurons = 4
    ...
    def log_inputs(self, snk_body, fd_pos):
        ...
        for z in range(self.maximum_data_size - len(self.inputs)):
